...
def graph_creator(cutoff):
    print("cutoff", cutoff)
    stations = pd.read_pickle('chosenStations.pkl')
...
class OriginalModel_cnn(nn.Module):
    def __init__(self, trace_len=1000):
    ...
    def forward(self, wav_input, graph_features):
...
class Model_cnn_for_embedding(nn.Module):
    def __init__(self, trace_len=1000):
        ...
        if self.model_chosen == 'nofeatures':
            self.merged_dense = nn.Linear(5120, 128)
        if self.model_chosen == 'main':
            self.merged_dense = nn.Linear(5120 + 78, 128)
