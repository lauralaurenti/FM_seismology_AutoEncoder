...
def normalize_for_emb(inputs):
    ...
    in_max = np.max(np.abs(inputs.reshape(inputs.shape[0],-1)), axis=1, keepdims=True)
...
def pre_post_equal_length(df_pre, df_visso, df_post, force_in_test="NO", num_classes=2):
...
def train_val_test_split(df, train_percentage=0.70, val_percentage=0.10, test_percentage=0.20, force_in_test=[], split_random=True, plot_hist=False, shuffle_df=True):
  ...
  if plot_hist:
      len_label=len(df_label.columns)
      train_df['binary'] = train_df.apply(lambda row: row.label.index(1), axis=1)
      val_df['binary'] = val_df.apply(lambda row: row.label.index(1), axis=1)
      test_df['binary'] = test_df.apply(lambda row: row.label.index(1), axis=1)
      plt.hist(train_df['binary'], label='train', bins=len_label)
      plt.hist(test_df['binary'], label='test', bins=len_label)
      plt.hist(val_df['binary'], label='val', bins=len_label)
      plt.legend(loc='upper right')
      plt.show()
      if not split_random:
            df_tra=df.iloc[index_train.flatten()]
            df_val=df.iloc[index_val.flatten()]
            df_te=df.iloc[index_test.flatten()]
            plt.hist(df_tra['trace_start_time'], bins=80)
            plt.hist(df_val['trace_start_time'], bins=80)
            plt.hist(df_te['trace_start_time'], bins=80)
...
def create_dataloader(X, y, index, target_dataset, batch_size = 32, normalization=True):
...
class CNN(nn.Module): 
    ...
    def forward(self, batch_init, batch_size, steps_in, softmax=False): 
        x = self.relu(self.maxpool(self.BatchNorm1(self.conv1(batch_init.permute(0,2,1))))) 
        x_1=x.clone()
...
class CNN_small(nn.Module):
    ...
    def forward(self, x, batch_size, steps_in, softmax=False): 
        x = self.relu(self.maxpool(self.BatchNorm7(self.conv7(x))))
        x_1=x.clone()
        # x=self.AvgPool(x)
